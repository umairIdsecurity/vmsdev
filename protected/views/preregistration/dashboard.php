...
<ul class="user-function">
	...
	<?php 
	$email=Registration::model()->findByPk($session['id'])->email;
	$check=new RegistrationAsic();
	$existmode=$check->exists("( email = '{$email}' AND is_saved=1)");
	if($existmode)
	{
	?>
	<li class="btn btn-lg btn-block"><a href="<?php echo Yii::app()->createUrl('preregistration'); ?>">Complete Your Saved Asic Application</a></li>
	<?php 
	}
	?>
