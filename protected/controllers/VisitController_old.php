...
class VisitController extends Controller {
        ...
        return array(
            array('allow', // allow authenticated user to perform 'create' and 'update' actions
                'actions' => array('create',
                    ...
                    'update', 'detail', 'admin', 'view', 'exportFile', 'evacuationReport', 'evacuationReportAjax', 'DeleteAllVisitWithSameVisitorId', 'closeVisit', 'visitResetById','exportVisitorRecordsCSV'),
                'users' => array('@'),
            ),
            array('allow',
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Visit'])) 
        {
			...
			if($model->card_type==CardType::VIC_CARD_SAMEDATE)
			{
    ...
    public function actionDetail($id, $new_created=NULL) {
	  ...
	  Yii:: app()->end();*/
        ...
        if (isset($_POST['updateVisitorDetailForm']) && isset($_POST['Visitor'])) 
        {
			...
			if(isset($_POST['ASIC'])&& isset($_POST['Visitor_asic_expiry_container']))
			{
				...
				if($asic->asic_expiry!=date("Y-m-d", strtotime(str_replace('/', '-',$_POST['Visitor_asic_expiry_container']))))
				{
					$asic->asic_expiry=date("Y-m-d", strtotime(str_replace('/', '-',$_POST['Visitor_asic_expiry_container'])));
					$ifCheck=1;
				}
				if($ifCheck==1)
				{
