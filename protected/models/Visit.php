...
//    }
	...
	public function getDateRangeOptions(){
		if( $this->card_type== CardType::VIC_CARD_24HOURS && $this->date_check_in!=NULL){
		$this->date_check_out=DateUtil::formatDateForDisplay(DateUtil::parseDate($this->date_check_in)->add(new DateInterval("P1D")));
		//$this->max_end_date=DateUtil::formatDateForDisplay(DateUtil::parseDate($this->date_check_in)->add(new DateInterval("P1D")));
		}
		//echo $this->min_start_date;
		//Yii::app()->end();
		return [
	...
	public function setDefaultCheckInOutValues($visitCount){
		...
		if($this->visit_status==VisitStatus::PREREGISTERED && Yii::app()->controller->action->id=='detail')
		{
			$this->time_check_in  = DateUtil::formatUserLocalTime();
		}
		...
		if($this->isNullDate($this->date_check_out) || ($endDateUnfixed && $this->visit_status!=VisitStatus::PREREGISTERED) ){
			if(($this->card_type==CardType::VIC_CARD_MULTIDAY ) || ( $this->card_type==CardType::VIC_CARD_EXTENDED ) )
			{
				$this->date_out = $this->finish_date = DateUtil::formatUserLocalDateForDisplay();
			}
			else if ($this->card_type== CardType::VIC_CARD_24HOURS)
			{
				$this->date_out = DateUtil::formatUserLocalDateForDisplay();
			}
			else
	...
	public function setDateRangeValues($visitCount){
		...
		switch( $this->card_type) {
			...
			case CardType::VIC_CARD_MULTIDAY:
				$this->max_duration = min(28,$visitCount['remainingDays']-1);
				//echo $visitCount['totalVisits'];
				$this->photo_required_after_3_days = true;
				break;
			...
			case CardType::VIC_CARD_24HOURS:
				...
				//$this->date_check_out=$endDate;
				//$endTime= ;
				//$dateTime=$endDate.$endTime;
				$this->max_time=DateUtil::formatTimeforDisplay(DateUtil::parseTime($this->time_check_in));
				//echo $endDate;
				//echo $endTime;
				//echo $check;
		...
		if($this->visit_status==VisitStatus::PREREGISTERED || $this->visit_status==VisitStatus::SAVED){
			$this->max_start_date = null;
			$this->max_end_date = null;
			if($this->visit_status==VisitStatus::PREREGISTERED)
			{
				$this->min_start_date=Date('d/m/Y');
			}
	...
	public function search($merge = null) {
		...
		$criteria->with = array('card0','host0', 'visitor0', 'company0', 'reason0', 'workstation0','visitor1','company1','user0');
		...
		$criteria->compare('tenantCode.code', $this->tenantCode, true);
		...
		if ($this->filterProperties) {
			$query = "t.id LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR card0.card_number LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR company0.code LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.first_name LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.last_name LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.date_of_birth LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.contact_number LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.contact_street_no LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.contact_street_name LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.contact_suburb LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.contact_state LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.contact_postcode LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR reason0.reason LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR workstation0.name LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR company0.name LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR company0.contact LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR company0.mobile_number LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR company0.email_address LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.contact_street_type LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR finish_date LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR card_returned_date LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.identification_type LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.identification_document_no LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.identification_document_expiry LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.asic_no LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                OR visitor0.asic_expiry LIKE CONCAT('%', '{$this->filterProperties}' , '%')
				OR user0.email LIKE CONCAT('%', '{$this->filterProperties}', '%')";
			...
			switch($this->card_type)
			{
				...
				case CardType::MANUAL_VISITOR:
				case CardType::CONTRACTOR_VISITOR:
					$query .= " OR host0.first_name LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                    OR host0.last_name LIKE CONCAT('%', '{$this->filterProperties}' , '%')";
					break;
				default:
					$query .= " OR visitor1.first_name LIKE CONCAT('%', '{$this->filterProperties}' , '%')
                    OR visitor1.last_name LIKE CONCAT('%', '{$this->filterProperties}' , '%')";
					break;
			}
			$criteria->addCondition($query);
			//$criteria->params = array(''{$this->filterProperties}'' => $this->filterProperties);
		}
	...
	public function relations() {
		...
		return array(
			...
			'company1'=>array(
				self::BELONGS_TO, 'Company', array('tenant' => 'id')
			),
			'user0'=>array(self::BELONGS_TO, 'User', array('created_by'=>'id')),
	...
	public function updateVisitsToClose() {
		...
		try {
			...
			$command = Yii::app()->db->createCommand("UPDATE visit
                    ...
                    AND card_status = " . CardStatus::ACTIVE . " and (card_type= " . CardType::SAME_DAY_VISITOR . " OR card_type=" . CardType::TEMPORARY_ASIC . ")")->execute(); //12/10/2017
	...
	public function updateTemporaryAsicVisitsToExpired() { //12/10/2017
		try {
			$command = Yii::app()->db->createCommand(
				"UPDATE visit SET visit_status = " . VisitStatus::EXPIRED
				. ", card_option =" . CardStatus::RETURNED
				. ", finish_date = CURRENT_DATE , finish_time = CURRENT_TIME
                    ...
                    AND CURRENT_TIME > time_check_out
                    AND visit_status = " . VisitStatus::ACTIVE . "
                    AND card_type = " . CardType::TEMPORARY_ASIC . "")->execute();
	...
	public function getVisitCount($visitId){
		...
		else if ($visitCount>28) 
		{
			return ['allVisitsByVisitor' => 28,
		        'companyVisitsByVisitor' => 28,
		        'remainingDays' => max(0,$remainingDays),
		        'companyName' => ''];
	...
	public function getRemainingDays()
	{
		...
		switch ($this->card_type) {
			...
			case CardType::VIC_CARD_24HOURS:
			case CardType::VIC_CARD_SAMEDATE:
			case CardType::TEMPORARY_ASIC:  //check visit count for Temporary asic
	...
	public function setExpireOrClosedVisits( $tenant_Id, $visit_id = "" )
	{
		...
		if( $visits )
		{
			...
			foreach( $visits as $key => $visit )
			{
				...
				// Expire same day visit on end of day that is yyyy-mm-dd HH:ii:ss @Manish.
				$dateIn  = new DateTime($visit["date_check_in"] . ' ' . $visit["time_check_in"]);
				$dateOut = new DateTime($visit["date_check_out"] . ' ' . '23:59:59');

				$dateNow = new DateTime("NOW", new DateTimeZone($timezone));
				$isExpired = $dateOut->diff($dateNow)->format("%r%d%h");
				$isExpired=(int)$isExpired;
				  // $dStart = new DateTime('2012-08-26 17:59:59');
				//	$dEnd  = new DateTime('2012-08-26 22:59:59');
					
				//echo "<pre>";
				//print_r($dEnd->diff($dStart)->format("%r%d%h"));
				//echo "</pre>";
				
				
				if( $isExpired >= 0 && is_int($isExpired) )
